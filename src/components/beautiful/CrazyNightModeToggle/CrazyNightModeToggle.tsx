...
import styles from './CrazyNightModeToggle.module.scss';
...
type CrazyNightModeToggleProps = {
  isChecked: boolean;
...
export default function CrazyNightModeToggle(props: CrazyNightModeToggleProps) {
  const { isChecked, setIsChecked, disabled, className } = props;

  return (
    <div className={clsx(styles.toggleCont, className)}>
      <label className={styles.toggleLabel}>
        ...
        <div className={styles.contIcon}>
          {starsArr.map((style, index) => (
            <span key={index} style={style as any} className={styles.sparkle}></span>
          ))}

          <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 30 30' className={styles.icon}>
